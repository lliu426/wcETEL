...
def run_mass_update(X_data,massInit,density,eta,lam = 1,verbose = False,gMethod = "secondOrder", numGstepsPerIter = 1,maxiter = 10000,tol = 1e-10, lamMethod = "absolute",illConditionThresh = 10e5):
    ...
    for i in range(maxiter):
        if verbose: print("    The mass update iteration "+str(i)+" starts.")
        negativePotential = optimal_transport_V2.optimal_transport(density,X_data,masses,negativePotential,maxiter = numGstepsPerIter,learningRate = eta,illConditionThresh = illConditionThresh,method=gMethod,verbose=verbose,beta=beta)
        ...
        errors[i] = err
        if err <= tol:
            if verbose: print("    Mass update finished at iteration "+str(i)+".")
            break
        masses = masses1
...
def mass_update_repeated_runner(X_data,numWeightInits,concentrationInit,density,lamMethod,eta,lam,
                                numGstepsPerIter = 1,maxiter = 2000,tol = 1e-7,verbose=False,
                                traceCanvas=None,upperLimOnTracePlot=None):
    """
    Robust version: one failed repetition is logged and skipped; the rest continue.
    On failure we set per-rep metrics to +inf so they don't affect 'best' selection.
    """
    n = len(X_data)
    bestWeights = np.zeros(n)
    bestObjective = np.inf
    objVals = np.full(numWeightInits, np.inf, dtype=float)
    wassers = np.full(numWeightInits, np.inf, dtype=float)
    klCosts = np.full(numWeightInits, np.inf, dtype=float)

    for j in np.arange(numWeightInits):
        if verbose: print(f"\n\nThe {j+1}th repeated runner starts.")
        try:
            massInit = np.random.dirichlet(concentrationInit)

            # run the inner alternating updates
            masses, g, err, numIter, errors = run_mass_update(
                X_data, massInit, density, eta, lam=lam, verbose=verbose,
                gMethod="secondOrder", numGstepsPerIter=numGstepsPerIter,
                maxiter=maxiter, tol=tol, lamMethod=lamMethod
            )

            # If inner routine signals NA, skip this repetition
            if isinstance(masses, str) and masses == "NA":
                if verbose:
                    print(f"[WARN] λ={lam}: repetition {j+1} returned NA; skipping this repetition.")
                continue

            # compute objective components
            wasserCost = optimal_transport_V2.computeW2_squared(X_data, masses, density)
            # If computeW2_squared returns inf (e.g., stuck), treat as failure
            if not np.isfinite(wasserCost):
                if verbose:
                    print(f"[WARN] λ={lam}: repetition {j+1} W2^2=inf; skipping this repetition.")
                continue
            ...
            if lamMethod == "no_KL":
                ...
                objVal = KLcost + lam*wasserCost
            else:
                # unknown mode -> treat as failure for safety
                if verbose:
                    print(f"[WARN] λ={lam}: repetition {j+1} unknown lamMethod={lamMethod}; skipping.")
                continue

            # record per-rep metrics
            objVals[j] = objVal
            ...
            # optional trace plot for successful reps only
            if traceCanvas and np.isfinite(objVal):
                traceCanvas.plot(np.sort(masses), alpha=0.3)
                if upperLimOnTracePlot is not None:
                    traceCanvas.set_ylim((0, upperLimOnTracePlot))
                traceCanvas.set_title("Lambda = " + str(lam) +
                                      ": weights in increasing order \n (all successful runs plotted)")

        except Exception as e:
            # log and skip this repetition, continue others
            if verbose:
                print(f"[WARN] λ={lam}: repetition {j+1} failed with: {e}")
                import traceback
                traceback.print_exc()

            # leave objVals/wassers/klCosts[j] at +inf so it won't affect best

            continue  # move on to the next repetition

    # If every rep failed, keep previous behavior (bestWeights=0 vector, bestObjective=inf).
    # Callers typically handle this by seeing large objective or empty plots.
