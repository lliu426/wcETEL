...
for i in range(n_datasets):
    random.seed(100)
#     sig = np.random.beta(2, 2, size=N - (2*n))
#     noi_1 = np.random.beta(1, 100, size = n)
#     noi_2 = np.random.beta(100, 1, size = n)
#     X = np.sort(np.concatenate([sig, noi_1, noi_2]))
    sig = np.random.normal(0, 1, size = N - n)
    noi = np.random.normal(10,1, size = n)
    X = np.sort(np.concatenate([sig, noi]))
    datasets.append(X)
...
class PowerDiagram1D:
    ...
    def update_boundaries(self):
        ...
        i0 = np.sum(Bounds_noconstr <= self.L_lb)
        iend = np.sum(Bounds_noconstr >= self.L_ub)
        indices = indices[i0:len(indices) - iend]
        self.indices = indices
        self.Bounds = np.empty(len(indices) + 1, dtype = float)
        self.Bounds[0] = self.L_lb
        self.Bounds[-1] = self.L_ub
        self.Bounds[1:-1] = Bounds_noconstr[i0:len(Bounds_noconstr) - iend]
        print("updated boundaries:",self.Bounds)
...
class OptimalTransport1D(PowerDiagram1D):
    ...
    def update_weights(self, tol=1e-6, maxIter=500, verbose=False):
        ...
        if self.intp_rho is None:
            print(compute_integrals(self.rho))
